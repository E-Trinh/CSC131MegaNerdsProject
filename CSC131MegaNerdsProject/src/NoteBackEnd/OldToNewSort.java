...
public class OldToNewSort implements NoteSort{
	...
	public ArrayList<Note> sort(ArrayList<Note> notes){
		return compare(notes);																					// returned sorted arraylist of notes
	}
	...
	Note noteTemp;
		...
		for (int i = 0; i < toSort.size(); i++) {																// outer loop of bubble sort
			for( int j = 0; j < toSort.size() - i - 1; j++) {													// inner loop of bubble sort
				if (toSort.get(i).getModification().isBefore(toSort.get(i+1).getModification()))				// if last modification date of note1 is less than last mod date of note2
				{
